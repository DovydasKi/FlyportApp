//
//  RegistrationTableViewModelTests.swift
//  FlyportAppTests
//
...
class RegistrationTableViewModelTests: XCTestCase {
	...
	public func testFlightNumberTitle() {
		...
		let viewModel = RegistrationTableViewModel(point: point, info: flightInfo)
		
		XCTAssertEqual(viewModel.flightNumber, "11")
	}
	
	public func testYourRegistrationTableTitle() {
		let flightInfo = self.flightInfo(id: 11, flightNumber: "11")
		let point = self.airportPoint(id: 1, pointNumber: "1")
		let viewModel = RegistrationTableViewModel(point: point, info: flightInfo)
		let expectedResult = LocalizationKeys.yourRegistrationTable.localized()
		
		XCTAssertEqual(viewModel.yourRegistrationTable, expectedResult)
	}
	...
	public func testNavigateToTableTitle() {
		...
		let point = self.airportPoint(id: 1, pointNumber: "1")
		let viewModel = RegistrationTableViewModel(point: point, info: flightInfo)
		let expectedResult = LocalizationKeys.navigateToRegistration.localized()
	...
	public func testGetRegistrationPoint() {
		let flightInfo = self.flightInfo(id: 11, flightNumber: "11")
		let point = self.airportPoint(id: 1, pointNumber: "1")
		let viewModel = RegistrationTableViewModel(point: point, info: flightInfo)
		
		XCTAssertEqual(viewModel.getRegistrationPoint().airportPointId, 1)
	}
	
	public func testGetSeurityPointInfoWhenResponseIsNil() {
		UserDefaults.standard.setUserFlightId(value: 123)
		let serviceMock = AirportPointServiceMock(point: nil)
		let flightInfo = self.flightInfo(id: 11, flightNumber: "11")
		let point = self.airportPoint(id: 1, pointNumber: "1")
		let viewModel = RegistrationTableViewModel(point: point, info: flightInfo, service: serviceMock)
	...
	public func testCompleteVisit() {
		...
		viewModel.completeRegistrationTableVisit {
			expectation.fulfill()
			XCTAssertTrue(true)
		}
...
extension RegistrationTableViewModelTests {
